...
export default function ScanToPdf() {
  ...
  const [rotationAngle, setRotationAngle] = useState(0); // Deskew
  const [contrast, setContrast] = useState(100);
  const [brightness, setBrightness] = useState(100);
  const [binarize, setBinarize] = useState(false); // Black & White Scanner mode
  const [backgroundColor, setBackgroundColor] = useState('#ffffff'); // PDF BG

  // ========== OCR SETTINGS ==========
  const [ocrText, setOcrText] = useState('');
  const [ocrLang, setOcrLang] = useState('eng'); // 'eng', 'hin', 'spa'
  const [ocrSuccess, setOcrSuccess] = useState(false);

  // ========== OUTPUT / PRO SETTINGS ==========
  const [outputFormat, setOutputFormat] = useState('pdf'); // 'pdf' | 'zip'
  ...
  const handleCameraCapture = (e) => {
    // Camera uses default file input with capture
  ...
  const addImageToItems = (file) => {
    ...
    setItems(prev => [...prev, {
      ...
      file, url,
      rotation: 0, 
      cropX: null, cropY: null, cropW: null, cropH: null // Placeholder for manual crop
    }]);
  };
  ...
  const performAutoCrop = (file) => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        const threshold = 230; // White background threshold

        for (let y = 0; y < canvas.height; y++) {
          for (let x = 0; x < canvas.width; x++) {
            const i = (y * canvas.width + x) * 4;
            // Check if pixel is NOT white (Paper edge)
  ...
  const processImageToCanvas = async (item, cropData = null) => {
    ...
    // 3. CONTRAST & BRIGHTNESS & BINARIZE & GRAYSCALE
    const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
    const data = imageData.data;
    ...
    // 4. SET DPI FOR PDF (We resize canvas based on DPI output)
    // If we are exporting to PDF, the resolution is tied to image pixels and PDF points.
    // We don't need to resize canvas in px, just multiply points in PDF.
  ...
  const triggerOcr = async () => {
    ...
    try {
      // 🛡️ Dynamic Import for Tesseract to prevent Vercel Build errors
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        const { data: { text } } = await Tesseract.recognize(imgDataUrl, ocrLang, {
  ...
  // ========== FINAL PROCESS (Generate PDF/ZIP + Security) ==========
  const processScan = async () => {
    ...
    try {
      ...
      for (let i = 0; i < items.length; i++) {
        // 1. Process each image with all Enhancements + DPI
        const item = items[i];
        const canvas = await processImageToCanvas(item);
        
        // DPI Calculation for PDF: Page Size in points = (Pixels / DPI) * 72
        ...
        // 3. Add BACKGROUND COLOR (if user wants filled page)
        ...
        if (watermarkText.trim()) {
          ...
          page.drawText(watermarkText, {
            x: 50, y: 50, size: 30, font,
            color: rgb(r, g, b), opacity: 0.4
          });
        }

        // 5. INJECT OCR TEXT (Hidden layer for Searchable PDF)
        ...
        // 6. PASSWORD PROTECT
        if (password) {
          pdfBytes = await pdfDoc.save({ password, userPassword: password });
        }
      ...
      <main className="flex-grow flex flex-col items-center p-6 mt-16 mb-10">
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Upload images, crop, adjust deskew/contrast, run OCR for searchable text, and export.
          </p>
        </div>
                    ...
                    {items.map((item, index) => (
                      ...
                      >
                        <div className="flex items-center gap-4 flex-1 min-w-0">
                          ...
                          <div className="w-10 h-10 bg-gray-100 rounded flex items-center justify-center shrink-0 border">
                            <img src={item.url} className="w-full h-full object-cover rounded" alt="Thumb" />
                          </div>
                  ...
                  {activeTab === 'images' && (
                    <div className="flex-grow overflow-y-auto space-y-4">
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-3">
                        ...
                        <p className="text-xs text-gray-500">Drag cards above to manually reorder images.</p>
                      </div>
                    </div>
                  )}

                  {/* 2. ENHANCE TAB - Pro Image Processing */}
                  {activeTab === 'enhance' && (
                    <div className="flex-grow overflow-y-auto space-y-4">
                  ...
                  {activeTab === 'ocr' && (
                    <div className="flex-grow overflow-y-auto space-y-4">
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-3">
                        ...
                        <p className="text-xs text-gray-500">OCR text will be hidden inside the PDF to make it searchable via Ctrl+F.</p>
                      </div>
                    </div>
                  )}

                  {/* 4. OUTPUT TAB - DPI, Color, Watermark, Password */}
                  {activeTab === 'output' && (
                    <div className="flex-grow overflow-y-auto space-y-4">
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-4">
                        ...
                        <div className="border-t pt-3"><label className="text-xs font-bold text-gray-800 block mb-1">Password Protect (Optional)</label>
