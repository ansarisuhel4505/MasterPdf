...
const translations = {
  en: {
    ...
    desc: 'Redact text, images, and areas from PDF documents permanently.',
  ...
  hi: {
    ...
    desc: 'PDF दस्तावेज़ों से टेक्स्ट, छवियाँ, और क्षेत्र स्थायी रूप से redact करें।',
...
export default function RedactPdf() {
  ...
  // 🔥 FIX: 100% Accurate Coordinates + Reverse Drawing Logic
  ...
  const handleMouseMove = (e) => {
    ...
    setBoxes(prev => {
      ...
      // Handles drawing backwards (up or left)
  ...
  const markAllFound = () => {
    ...
    const newBoxes = findResults.map(result => ({
        ...
        <div className="flex flex-col md:flex-row gap-6 w-full max-w-7xl mx-auto">
          ...
          <div className={`md:w-72 w-full p-4 rounded-2xl border shadow-sm ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">{t.overlayOpacity} ({options.overlayOpacity}%)</label>
              ...
              <button onClick={() => setShowAdvanced(!showAdvanced)} className="w-full flex items-center justify-center gap-2 py-2 bg-blue-500 text-white rounded-lg">
                {showAdvanced ? <ChevronUp size={18} /> : <ChevronDown size={18} />} {t.advanced}
              </button>

              {showAdvanced && (
                <div className="space-y-3">
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={options.sanitizeMetadata} onChange={(e) => setOptions({ ...options, sanitizeMetadata: e.target.checked })} /> {t.sanitizeMetadata}
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={options.removeComments} onChange={(e) => setOptions({ ...options, removeComments: e.target.checked })} /> {t.removeComments}
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={options.flattenForms} onChange={(e) => setOptions({ ...options, flattenForms: e.target.checked })} /> {t.flattenForms}
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={options.removeLayers} onChange={(e) => setOptions({ ...options, removeLayers: e.target.checked })} /> {t.removeLayers}
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={options.removeAttachments} onChange={(e) => setOptions({ ...options, removeAttachments: e.target.checked })} /> {t.removeAttachments}
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={options.reversible} onChange={(e) => setOptions({ ...options, reversible: e.target.checked })} /> {t.reversible}
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={options.retentionEphemeral} onChange={(e) => setOptions({ ...options, retentionEphemeral: e.target.checked })} /> {t.retentionEphemeral}
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={options.smartFilename} onChange={(e) => setOptions({ ...options, smartFilename: e.target.checked })} /> {t.smartFilename}
                  </label>
                  <label className="flex items-center gap-2 text-sm font-bold text-blue-600 dark:text-blue-400">
                    <input type="checkbox" checked={options.aiEnabled} onChange={(e) => setOptions({ ...options, aiEnabled: e.target.checked })} /> <Sparkles size={14}/> {t.aiDetection}
                  </label>
                </div>
              )}
            </div>
            ...
            <div className={`rounded-2xl shadow-sm border p-6 min-h-[600px] flex flex-col items-center ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              {!file ? (
                <div 
                  ...
                  onDrop={handleDrop} 
                  className="flex-1 w-full border-2 border-dashed rounded-xl p-10 flex flex-col items-center justify-center text-center"
                >
                  ...
                  <button onClick={() => fileInputRef.current.click()} className="bg-[#E5322D] text-white px-8 py-3 rounded-xl font-bold mt-4 hover:bg-red-700 transition">
                    {t.browse}
                  </button>
                </div>
              ) : (
                <div className="flex-1 w-full flex flex-col">
                  ...
                  <div className="flex flex-wrap justify-between items-center mb-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={clearBoxes} className="px-3 py-1.5 rounded-lg text-xs bg-red-100 text-red-600 hover:bg-red-200 transition font-bold">
                        {t.clearBoxes}
                      </button>
                      <button onClick={clearAll} className="px-3 py-1.5 rounded-lg text-xs bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 transition flex items-center gap-1 font-bold">
                  ...
                  <div className="flex gap-2 mb-4">
                    ...
                    <button onClick={handleFind} className="px-4 py-2 bg-gray-800 text-white rounded-lg font-bold text-sm">Find</button>
                    <button onClick={markAllFound} className="px-4 py-2 bg-yellow-500 text-white rounded-lg font-bold text-sm">{t.markAll}</button>
                  </div>

                  {/* PDF Viewer - Tightly wrapped to fix coordinates */}
                  <div className="relative flex-1 overflow-auto bg-gray-100 dark:bg-gray-900 p-4 rounded-lg flex justify-center border border-gray-200 dark:border-gray-800">
                    <div 
                      ...
                      style={{ width: 'max-content', height: 'max-content' }} // 🔥 Tight wrapper fix
                    >
                  ...
                  <button
                    ...
                    className="mt-6 py-4 bg-[#E5322D] text-white font-bold text-lg rounded-xl flex justify-center items-center gap-2 disabled:bg-gray-400 hover:bg-red-700 transition shadow-md"
                  >
                    {isProcessing ? <><Loader2 className="animate-spin" /> {t.redacting} {progress}%</> : <><Shield size={24} /> {t.applyRedaction}</>}
                  </button>
                </div>
              )}
            </div>
