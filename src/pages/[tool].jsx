  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/master-convert', {
        ...
        body: JSON.stringify({ action: tool, fileUrl: blob.url }),
      });
          ...
          {!file ? (
            <div className="text-center w-full">
              ...
              <label htmlFor="file-upload" className="cursor-pointer bg-[#E5322D] hover:bg-red-700 text-white text-xl font-bold py-6 px-12 rounded-xl inline-flex items-center gap-3 transition shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                <UploadCloud size={28} /> Select file for {formattedToolName}
              </label>
                ...
                <div>
                  ...
                  {success ? (
                    ...
                    <p className="text-sm text-gray-500 leading-relaxed bg-gray-50 p-4 rounded-lg">
                      Your file is loaded and ready. Click the button below to execute <strong>{formattedToolName}</strong> securely.
                    </p>
                  )}
                </div>

                <div className="mt-6 flex justify-end">
                   <button onClick={handleProcess} disabled={isProcessing || success} className="w-full flex items-center justify-center gap-2 px-8 py-4 rounded-xl text-white font-bold text-lg transition shadow-md bg-[#E5322D] hover:bg-red-700 hover:shadow-lg disabled:bg-gray-400">
                     {isProcessing ? <><Settings className="animate-spin" size={24} /> Processing...</> : success ? <>Done Successfully</> : <>Execute {formattedToolName} <ArrowRight size={24} /></>}
                   </button>
                </div>
              </div>
            </div>
