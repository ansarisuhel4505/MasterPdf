...
export default function PdfToWord() {
  ...
  const convertToWord = async () => {
    ...
    try {
      ...
      if (response.ok && data.downloadUrl) {
        ...
        link.download = `MasterPdf_Word_${file.name.split('.')[0]}.docx`;
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-[#F5F5F7]">
      <Head>
        <title>Convert PDF to Word online - MasterPdf</title>
      </Head>
      <Navbar />
            ...
            <div className="w-full h-full flex flex-col md:flex-row gap-8 items-start pt-4">
              ...
              <div className="w-full md:w-1/2 flex flex-col items-center justify-center bg-gray-50 border border-gray-200 rounded-lg p-8 relative h-[350px]">
                ...
                <div className="relative">
                  ...
                  <FileCode2 size={60} className="absolute -right-28 top-1/2 transform -translate-y-1/2 text-blue-600 opacity-90" />
                </div>
                <p className="text-sm text-gray-800 font-bold text-center break-words w-full px-4 mt-8">
                  {file.name}
                </p>
                <p className="text-xs text-gray-500 mt-2">
                  Ready to convert to Word (DOCX)
                </p>
              </div>
                    ...
                    <label className="flex items-start p-4 border border-[#E5322D] bg-red-50 rounded-lg cursor-pointer transition">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-gray-900">Convert to DOCX</span>
                        </div>
                        <p className="text-xs text-gray-500 mt-1">Extract text and layout maintaining the original formatting.</p>
                      </div>
                    </label>
                  </div>
                </div>
