...
import { 
  ...
  ChevronRight, ShieldCheck, Upload, QrCode, Palette,
  Layers, Globe, ArrowLeft, ArrowRight, HardDrive, Link2, Trash2,
  ListOrdered, Users, MessageSquare, Shield, ImageIcon, ScanText, GripVertical, CheckCircle2,
  ChevronUp, ChevronDown, Plus, Monitor, ArrowRightCircle
} from 'lucide-react';
...
export default function VisualSignPdf() {
  ...
  // App Flow: 1(Upload) -> 1.5(Who) -> 'request_form'(Multiple) -> 2(Editor Workspace) -> 4(Download)
  ...
  const removeFile = () => {
    setFile(null); setFileUrl(null); setElements([]); setCurrentPage(1); setStep(1); setIsMultiSigner(false);
  ...
  const addElement = (type) => {
    ...
    if (isMultiSigner) {
      ...
      if (!signer) return;
      const displayValue = type === 'signature' ? 'Signature' : type === 'initials' ? 'Initials' : type === 'name' ? 'Name' : type === 'date' ? 'Date' : type === 'text' ? 'Text' : 'Company Stamp';
      const newElement = {
        id: Date.now(), type, page: currentPage, x: 100, y: 150, width: 160, height: 50,
        value: displayValue, isPlaceholder: true, signerId: signer.id, signerName: signer.name || signer.email || 'Signer', colorIdx: signer.colorIdx
      };
  ...
  const applySignatureAndDownload = async () => {
    ...
    if (elements.length === 0) {
      alert("Please place at least one signature or field on the document.");
      return;
    }
    
    if (isMultiSigner) {
      setIsProcessing(true);
      setTimeout(() => { setIsProcessing(false); setStep('request_sent'); }, 2000);
      return;
    }
  ...
  const pageUp = () => setCurrentPage(prev => Math.max(prev - 1, 1));
  const pageDown = () => setCurrentPage(prev => Math.min(prev + 1, numPages));
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-[#F5F5F7]">
      ...
      <main className="flex-grow flex flex-col items-center justify-center pt-24 pb-10 px-4">
        ...
        {step === 1.5 && (
          <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="flex flex-col md:flex-row p-8 gap-8 bg-gray-50">
              <div onClick={() => { setIsMultiSigner(false); setStep(2); setShowSignatureModal(true); }} className="flex-1 bg-white border border-gray-200 rounded-xl p-8 flex flex-col items-center text-center cursor-pointer hover:shadow-xl hover:border-[#E5322D] transition-all group">
        ...
        {/* STEP 2: UNIFIED EDITOR WORKSPACE (FOR SOLO AND MULTI-SIGNER) */}
        {step === 2 && (
          <div className="w-full max-w-[1600px] h-[85vh] flex flex-col bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden animate-in fade-in">
            ...
            {/* 🔥 NEW TOP BAR (EXACT ILOVEPDF LAYOUT) */}
            <div className="h-14 bg-gray-50 border-b border-gray-200 flex items-center px-4 gap-4 shrink-0">
               <div className="flex items-center gap-1 border border-gray-300 rounded bg-white">
                 <button onClick={pageUp} disabled={currentPage <= 1} className="p-1.5 text-gray-500 hover:text-gray-800 disabled:opacity-30 border-r border-gray-300"><ChevronUp size={16}/></button>
                 <button onClick={pageDown} disabled={currentPage >= numPages} className="p-1.5 text-gray-500 hover:text-gray-800 disabled:opacity-30"><ChevronDown size={16}/></button>
               </div>
               <div className="flex items-center gap-2 text-sm font-medium text-gray-600">
                 <input type="text" value={currentPage} readOnly className="w-8 text-center border border-gray-300 rounded py-1" />
                 <span>/ {numPages}</span>
               </div>
               <div className="h-6 w-px bg-gray-300 mx-2"></div>
               <div className="flex items-center gap-2 border border-gray-300 bg-white rounded px-3 py-1.5 cursor-pointer hover:bg-gray-50 transition">
            ...
            <div className="flex-grow flex flex-col lg:flex-row overflow-hidden">
              ...
              {/* Sidebar - Small Page Thumbnails */}
              <div className="w-full lg:w-32 bg-gray-100 border-r border-gray-200 p-4 flex flex-col items-center gap-4 overflow-y-auto hidden lg:flex shrink-0">
                 {Array.from({ length: numPages }, (_, i) => i + 1).map(page => (
                   <div key={page} className="flex flex-col items-center gap-1">
                     <div 
                       onClick={() => setCurrentPage(page)}
                       className={`w-20 bg-white shadow-sm border-2 overflow-hidden cursor-pointer transition-all flex items-center justify-center min-h-[28px] ${currentPage === page ? 'border-[#E5322D]' : 'border-gray-200 hover:border-gray-300'}`}
                     >
                        {/* Render miniature page thumbnail for performance */}
                        <Document file={fileUrl}>
                          <Page pageNumber={page} width={76} renderTextLayer={false} renderAnnotationLayer={false} />
                        </Document>
                     </div>
              ...
              <div className="flex-grow bg-[#EFEFEF] p-6 flex flex-col items-center justify-start overflow-y-auto relative border-r border-gray-200">
                 ...
                 {/* Mobile Pagination control fallback */}
                 <div className="lg:hidden flex items-center justify-between w-full max-w-[600px] mb-4 bg-white p-2 rounded-lg shadow-sm border border-gray-200">
                   <button disabled={currentPage <= 1} onClick={pageUp} className="p-2 text-gray-600 disabled:opacity-50"><ChevronLeft size={20}/></button>
                   <span className="font-bold text-sm text-gray-800">Page {currentPage} of {numPages}</span>
                   <button disabled={currentPage >= numPages} onClick={pageDown} className="p-2 text-gray-600 disabled:opacity-50"><ChevronRight size={20}/></button>
                 </div>
                 ...
                 <div className="relative shadow-xl bg-white select-none">
                   ...
                   {/* 🔥 FLOATING ACTION BUTTON (FAB) on Right edge of document */}
                   <div className="absolute -right-14 top-10 flex flex-col items-center gap-3 group z-20">
                     <button className="w-10 h-10 bg-[#E5322D] rounded-full flex items-center justify-center text-white shadow-lg z-10 transition-transform group-hover:rotate-45">
                       <Plus size={24} />
                     </button>
                     <div className="absolute top-12 flex flex-col gap-3 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto">
                       <button onClick={() => alert('Fit to screen clicked!')} className="w-10 h-10 bg-[#E5322D] hover:bg-red-700 rounded-full flex items-center justify-center text-white shadow-lg transition-transform hover:scale-110" title="Fit to Screen"><Monitor size={18}/></button>
                       <button onClick={() => window.open('https://drive.google.com/', '_blank')} className="w-10 h-10 bg-[#E5322D] hover:bg-red-700 rounded-full flex items-center justify-center text-white shadow-lg transition-transform hover:scale-110" title="Google Drive"><HardDrive size={18}/></button>
                       <button onClick={() => window.open('https://dropbox.com/', '_blank')} className="w-10 h-10 bg-[#E5322D] hover:bg-red-700 rounded-full flex items-center justify-center text-white shadow-lg transition-transform hover:scale-110" title="Dropbox"><Layers size={18}/></button>
                     </div>
                   </div>

                   {/* Elements Rendering */}
                   {elements.filter(el => el.page === currentPage).map((el) => {
                     const colorData = isMultiSigner ? (signerColors[el.colorIdx] || signerColors[0]) : null;
                     ...
                     return (
                       <Rnd
                         ...
                         className={`group border-2 border-dashed flex items-center justify-center transition-colors touch-none cursor-move ${isMultiSigner ? `${colorData.border} ${colorData.fill}/70 hover:${colorData.fill}` : 'border-transparent hover:border-gray-400 focus-within:border-[#E5322D] bg-white/50 hover:bg-white/80'}`}
                       >
            ...
            <div className="w-full lg:w-[350px] bg-white flex flex-col h-full shrink-0">
              ...
              <div className="p-6 overflow-y-auto flex-grow">
                ...
                <div className="mb-8">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {!isMultiSigner && sigMode === 'simple' && <button onClick={() => setShowSignatureModal(true)} className="text-[#E5322D] text-xs font-bold hover:underline flex items-center gap-1"><PenTool size={12}/> Edit</button>}
                  </div>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      {isMultiSigner ? (
                        <>
                          <div className="bg-gray-400 p-2 rounded text-white"><PenTool size={16}/></div>
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-2">
                    <button onClick={() => addElement('initials')} className="w-full border border-gray-200 rounded-lg p-3 bg-white hover:bg-gray-50 flex items-center justify-between transition-colors shadow-sm">
                      ...
                      {!isMultiSigner && <span className="text-lg mr-2" style={{ fontFamily: signatureFonts[selectedStyle], color: sigColor }}>{initials}</span>}
                    </button>
                    <button onClick={() => addElement('name')} className="w-full border border-gray-200 rounded-lg p-3 bg-white hover:bg-gray-50 flex items-center gap-3 transition-colors text-sm font-medium text-gray-700 shadow-sm">
                      <div className="bg-gray-400 p-1.5 rounded text-white"><User size={14}/></div> Name
                    </button>
                    <button onClick={() => addElement('date')} className="w-full border border-gray-200 rounded-lg p-3 bg-white hover:bg-gray-50 flex items-center gap-3 transition-colors text-sm font-medium text-gray-700 shadow-sm">
                      <div className="bg-gray-400 p-1.5 rounded text-white"><Calendar size={14}/></div> Date
                    </button>
                    <button onClick={() => addElement('text')} className="w-full border border-gray-200 rounded-lg p-3 bg-white hover:bg-gray-50 flex items-center gap-3 transition-colors text-sm font-medium text-gray-700 shadow-sm">
                      <div className="bg-gray-400 p-1.5 rounded text-white"><Type size={14}/></div> Text
                    </button>
              ...
              <div className="p-6 border-t border-gray-200 bg-gray-50 flex flex-col items-center shrink-0">
                {isMultiSigner ? (
                  <>
        ...
        {/* STEP 4: DOWNLOAD SCREEN WITH PROPER ICONS */}
        {step === 4 && (
          <div className="w-full max-w-4xl flex flex-col items-center justify-center animate-in slide-in-from-bottom-8 fade-in text-center mt-10">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <div className="grid grid-cols-2 gap-2 mt-4 sm:mt-0">
                <button onClick={() => window.open('https://drive.google.com/', '_blank')} className="bg-[#E5322D] hover:bg-red-700 text-white p-3 rounded-full shadow-md transition-transform hover:scale-105" title="Save to Google Drive"><HardDrive size={20}/></button>
                ...
                <button onClick={() => window.open('https://dropbox.com/', '_blank')} className="bg-[#E5322D] hover:bg-red-700 text-white p-3 rounded-full shadow-md transition-transform hover:scale-105" title="Save to Dropbox"><Layers size={20}/></button>
